...
import { Search, Filter, Plus, Download, Pencil, Trash2, X } from 'lucide-react';
...
const STATUS_OPTIONS: Array<EventStatus | 'All Statuses'> = ['All Statuses', 'Published', 'Draft', 'Review', 'Planning'];
...
export default function Events() {
  ...
  const [statusFilter, setStatusFilter] = useState<EventStatus | 'All Statuses'>('All Statuses');
  ...
  const resetForm = () => {
    setEventForm({ name: '', date: '', location: '', type: '', status: 'Draft' });
    setEditingId(null);
    setIsCreating(false);
    setFormError('');
  };
  ...
  const saveEvent = () => {
    if (!eventForm.name.trim() || !eventForm.date.trim() || !eventForm.location.trim() || !eventForm.type.trim()) {
  ...
  const exportCsv = () => {
    const header = ['Event ID', 'Event Name', 'Type', 'Date', 'Location', 'Attendees', 'Revenue', 'Status'];
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <button onClick={exportCsv} className="px-4 py-2 bg-white border border-gray-200 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors flex items-center gap-2">
            <Download className="w-4 h-4" /> Export CSV
          </button>
          {canCreateEvents ? (
            <button onClick={openCreateForm} className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700 transition-colors flex items-center gap-2 shadow-sm">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col sm:flex-row gap-4 justify-between items-center">
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          <button onClick={() => { setSearchQuery(''); setStatusFilter('All Statuses'); }} className="px-3 py-2 bg-white border border-gray-200 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors flex items-center gap-2 w-full sm:w-auto justify-center">
            <Filter className="w-4 h-4" /> Filter
          </button>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as EventStatus | 'All Statuses')} className="px-3 py-2 bg-white border border-gray-200 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 w-full sm:w-auto">
      ...
      {(isCreating || editingId) && (
        <div className="bg-white border border-blue-100 rounded-xl p-4 md:p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{editingId ? 'Edit Event' : 'Create Event'}</h3>
          ...
          <div className="mt-4 flex items-center gap-2">
            <button onClick={saveEvent} className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-semibold hover:bg-blue-700">
              {editingId ? 'Update Event' : 'Create Event'}
            </button>
            <button onClick={resetForm} className="px-4 py-2 bg-white border border-gray-200 rounded-md text-sm font-semibold text-gray-700 hover:bg-gray-50">
              Cancel
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <table className="data-grid">
            ...
            <tbody>
                  ...
                  <td>
                    {canUpdateEvents || canDeleteEvents ? (
                      <div className="flex items-center gap-1">
                        {canUpdateEvents && (
                          <button onClick={() => openEditForm(event)} className="p-1 text-blue-500 hover:text-blue-700 rounded" title="Edit event">
                            <Pencil className="w-4 h-4" />
                          </button>
                        )}
                        {canDeleteEvents && (
                          <button onClick={() => deleteEvent(event.id)} className="p-1 text-rose-500 hover:text-rose-700 rounded" title="Delete event">
              ...
              {filteredEvents.length === 0 && (
                <tr>
                  <td colSpan={8} className="text-center text-sm text-gray-500 py-8">No events found for the selected filters.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between bg-gray-50/50">
          <div className="text-sm text-gray-500">
            Showing <span className="font-medium text-gray-900">{filteredEvents.length}</span> of <span className="font-medium text-gray-900">{events.length}</span> results
          </div>
