...
import { ROLE_STEP2_FIELDS } from "../public/Auth";
...
export default function Profile() {
  // Secondary roles state (simulate backend)
  ...
  const [roleFormStep, setRoleFormStep] = useState(1);
  const [roleFormError, setRoleFormError] = useState("");

  // Handle add role form field changes
  const handleRoleFormChange = (field, value) => {
    setRoleForm((prev) => ({ ...prev, [field]: value }));
  };

  // Handle add role form submit
  const handleRoleFormSubmit = (e) => {
    e.preventDefault();
    setRoleFormError("");
    // Simple validation: require all fields in ROLE_STEP2_FIELDS[selectedRole]
    const fields = ROLE_STEP2_FIELDS[selectedRole] || [];
    for (const field of fields) {
      if (field.kind === "input" && field.required && !roleForm[field.label]) {
        setRoleFormError(`Please fill ${field.label}`);
        return;
      }
      if (field.kind === "upload" && field.required && !roleForm[field.id]) {
        setRoleFormError(`Please upload ${field.label}`);
        return;
      }
      if (field.kind === "select" && field.required && !roleForm[field.label]) {
        setRoleFormError(`Please select ${field.label}`);
        return;
      }
    }
    // Save new role
    const updated = [...secondaryRoles, selectedRole];
    setSecondaryRoles(updated);
    localStorage.setItem("secondaryRoles", JSON.stringify(updated));
  ...
  const role = (getAuthRole() as AuthRole) || "Member";
  const storageKey = `profile-data-${role}`;

  const activeTier =
    localStorage.getItem(`active-membership-tier-${role}`) || "Bronze";
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              ...
              {role === "Member" && (
                <button
                  className="mt-2 px-5 py-2 bg-blue-600 text-white rounded-xl font-bold text-xs shadow hover:bg-blue-700"
            ...
            {showAddRole && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
                <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-lg relative">
                  ...
                  <h2 className="text-lg font-bold mb-4">Add a Role</h2>
                  <form onSubmit={handleRoleFormSubmit} className="space-y-4">
                    <div>
                      ...
                      <select
                        className="block w-full px-3 py-2 border border-gray-300 rounded-xl text-sm"
                        value={selectedRole}
                        onChange={(e) => {
                    ...
                    {/* Show form fields for selected role */}
                    {selectedRole && ROLE_STEP2_FIELDS[selectedRole] && (
                      <div className="space-y-3">
                        {ROLE_STEP2_FIELDS[selectedRole].map((field) => {
                          if (field.kind === "input") {
                            return (
                              <div key={field.label}>
                    ...
                    <button
                      type="submit"
                      className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold text-sm mt-2"
                    >
                      Add Role
                    </button>
                  </form>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {[
              ...
              <div
                key={i}
                className={`px-4 py-1.5 border border-gray-100 rounded text-[11px] font-bold shadow-sm ${item.color}`}
              >
                ...
                <div className="mt-6 flex flex-col gap-4">
                  ...
                  <div className="space-y-4">
                    {selectedPlans.map((plan, i) => (
                      <div
                        key={i}
                        className="flex flex-col leading-tight border-l-2 border-emerald-400 pl-4 py-1"
                      >
                        <span className="text-sm font-black text-gray-900">
                          {plan}
                        </span>
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
                          Active Verified Asset
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  {selectedPlans.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {selectedPlans.map((plan, i) => (
                        <div
                          key={i}
                          className="p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100 transition-all hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 group border-l-4 border-l-indigo-500"
                        >
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    {user.awards.map((award, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-4 p-5 rounded-2xl border border-gray-100 hover:bg-amber-50/30 transition-all border-l-4 border-l-amber-400"
                      >
                        <div className="w-10 h-10 bg-amber-100 rounded-xl flex items-center justify-center border border-amber-200">
                          <Award className="w-5 h-5 text-amber-600" />
                        </div>
                        <div>
                          <h4 className="text-[11px] font-bold text-gray-900 uppercase tracking-tight leading-none mb-1">
                            {award}
                          </h4>
                ...
                <section>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {user.interests.map((interest) => (
                      <span
                        key={interest}
                        className="px-4 py-2 bg-slate-50 text-slate-500 text-[10px] font-black uppercase tracking-widest rounded-xl border border-slate-100 hover:bg-white hover:border-blue-200 hover:text-blue-600 transition-all cursor-default shadow-sm hover:shadow-md"
                      >
                        {interest}
                      </span>
                    ))}
