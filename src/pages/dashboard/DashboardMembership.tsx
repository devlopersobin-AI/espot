...
type MembershipTier = 'None' | 'Silver' | 'Gold' | 'Diamond';
...
const DEFAULT_MANAGED_ACCOUNTS: ManagedAccount[] = [
  { id: 'ACC-001', name: 'Mila Member', role: 'Member', tier: 'Silver', subscription: 'Active', partnerAccess: false, franchiseAccess: false },
  { id: 'ACC-002', name: 'Parker Partner', role: 'Partner', tier: 'Gold', subscription: 'Active', partnerAccess: true, franchiseAccess: false },
  { id: 'ACC-003', name: 'Farah Franchise', role: 'Franchisee', tier: 'Diamond', subscription: 'Paused', partnerAccess: false, franchiseAccess: true },
  { id: 'ACC-004', name: 'Jace Jobseeker', role: 'Member', tier: 'None', subscription: 'Cancelled', partnerAccess: false, franchiseAccess: false },
];
...
export default function Membership() {
  ...
  const [selectedPlan, setSelectedPlan] = useState<string>(() => localStorage.getItem(planStorageKey) || 'Professional');
  const [tierSettings, setTierSettings] = useState<TierSetting[]>(() => {
    ...
    try {
      ...
      return Array.isArray(parsed) && parsed.length > 0 ? parsed : DEFAULT_TIER_SETTINGS;
    } catch {
      return DEFAULT_TIER_SETTINGS;
    }
  });
  const [managedAccounts, setManagedAccounts] = useState<ManagedAccount[]>(() => {
    ...
    try {
      ...
      return Array.isArray(parsed) && parsed.length > 0 ? parsed : DEFAULT_MANAGED_ACCOUNTS;
    } catch {
      return DEFAULT_MANAGED_ACCOUNTS;
    }
  });
  ...
  useEffect(() => {
    if (isAdmin) {
      localStorage.setItem('membership-tier-settings', JSON.stringify(tierSettings));
      localStorage.setItem('membership-managed-accounts', JSON.stringify(managedAccounts));
    }
  ...
  const setAccountValue = (accountId: string, update: Partial<ManagedAccount>) => {
    setManagedAccounts((prev) => prev.map((account) => (
      account.id === accountId ? { ...account, ...update } : account
    )));
  };

  const toggleTierEnabled = (tier: Exclude<MembershipTier, 'None'>) => {
    setTierSettings((prev) => prev.map((setting) => (
      setting.tier === tier ? { ...setting, enabled: !setting.enabled } : setting
    )));
  };

  const updateTierPrice = (tier: Exclude<MembershipTier, 'None'>, field: 'monthlyPrice' | 'annualPrice', value: string) => {
    ...
    <div className="space-y-8">
      ...
      {!isAdmin && (
        <p className="text-center text-sm text-slate-500 -mt-6">
          Current plan: <span className="font-semibold text-slate-900">{selectedPlan}</span>
        </p>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {plans.map((plan) => (
          ...
          >
            ...
            {canUpdateMembership ? (
              <button onClick={() => setSelectedPlan(plan.name)} className={`w-full py-3 px-4 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${
                plan.recommended 
                  ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-sm' 
                  : 'bg-white border border-gray-200 text-gray-900 hover:bg-gray-50'
              }`}>
                {selectedPlan === plan.name ? 'Selected Plan' : 'Select Plan'} <ArrowRight className="w-4 h-4" />
              </button>
            ) : (
        ...
        <div className="space-y-8 max-w-6xl mx-auto">
          <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-900">Membership Tier CMS</h3>
              <span className="text-xs uppercase tracking-wider font-semibold text-emerald-700 bg-emerald-50 px-2 py-1 rounded">Admin only</span>
            </div>
            <p className="text-sm text-gray-500 mb-5">Enable or disable Silver, Gold, and Diamond plans and adjust pricing for monthly or annual subscription cycles.</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {tierSettings.map((setting) => (
                <div key={setting.tier} className="border border-gray-200 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-lg font-bold text-gray-900">{setting.tier}</h4>
                    <button onClick={() => toggleTierEnabled(setting.tier)} className={`px-2.5 py-1 text-xs font-semibold rounded ${setting.enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>
                  ...
                  <div className="space-y-3">
                    <label className="block text-sm text-gray-600">
                      ...
                      <input
                        ...
                        onChange={(e) => updateTierPrice(setting.tier, 'monthlyPrice', e.target.value)}
                    ...
                    <label className="block text-sm text-gray-600">
                      ...
                      <input
                        ...
                        onChange={(e) => updateTierPrice(setting.tier, 'annualPrice', e.target.value)}
                        className="mt-1 w-full bg-gray-50 border border-gray-200 rounded-md px-3 py-2 text-sm"
                      />
                    </label>
                  </div>
          ...
          <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
            <h3 className="text-xl font-bold text-gray-900 mb-2">Account Access Control</h3>
            <p className="text-sm text-gray-500 mb-5">Grant or revoke membership tiers, subscription status, partnership access, and franchise access.</p>
                ...
                <tbody>
                  {managedAccounts.map((account) => (
                    <tr key={account.id} className="border-b border-gray-100">
                      ...
                      <td className="py-3">
                        <select value={account.tier} onChange={(e) => setAccountValue(account.id, { tier: e.target.value as MembershipTier })} className="bg-gray-50 border border-gray-200 rounded-md px-2 py-1.5 text-sm">
                          <option>None</option>
                      ...
                      <td className="py-3">
                        <select value={account.subscription} onChange={(e) => setAccountValue(account.id, { subscription: e.target.value as SubscriptionState })} className="bg-gray-50 border border-gray-200 rounded-md px-2 py-1.5 text-sm">
                      ...
                      <td className="py-3">
                        <button
                          onClick={() => setAccountValue(account.id, { partnerAccess: !account.partnerAccess })}
                      ...
                      <td className="py-3">
                        <button
                          onClick={() => setAccountValue(account.id, { franchiseAccess: !account.franchiseAccess })}
