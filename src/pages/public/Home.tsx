...
export default function Home() {
  ...
  // ── 3 trust stats ────────────────────────────────────────────────────────
  const stats = [
    { value: '300,000+', label: 'Active Members', icon: Users, iconCls: 'text-slate-900', bgCls: 'bg-slate-100' },
    { value: '2.4M', label: 'Points Earned', icon: TrendingUp, iconCls: 'text-slate-900', bgCls: 'bg-slate-100' },
    { value: '860+', label: 'Partners Worldwide', icon: Globe, iconCls: 'text-slate-900', bgCls: 'bg-slate-100' },
  ];

  // ── Platform modules ─────────────────────────────────────────────────────
  const modules = [
    { title: 'Membership', desc: 'Join 300K+ members, earn reward points, and access exclusive perks, events, and services.', icon: Users, path: '/membership', cta: 'Explore Membership' },
    { title: 'Partner Program', desc: 'Build strategic business alliances, co-market services, and grow revenue together.', icon: Handshake, path: '/partner', cta: 'Become a Partner' },
    { title: 'Franchise', desc: 'Own an E-Spot Club franchise in your city with proven systems and brand support.', icon: Building2, path: '/franchise', cta: 'Own a Franchise' },
    { title: 'Training', desc: 'Access certified courses, skill workshops, and career acceleration programs.', icon: GraduationCap, path: '/training', cta: 'Start Learning' },
    { title: 'Investment', desc: 'Early Bird, Premium & Elite investment tiers across 8 venture categories worldwide.', icon: TrendingUp, path: '/investment', cta: 'View Investment' },
    { title: 'Entrepreneurship', desc: 'Launch and scale your startup with mentorship, funding connects, and community.', icon: Zap, path: '/entrepreneurship', cta: 'Start Growing' },
    { title: 'Leadership', desc: 'Develop leadership skills through structured programs, workshops, and mentors.', icon: Award, path: '/leadership', cta: 'Lead Better' },
    { title: 'Scholar', desc: 'Connect with academic programs, scholarships, and international study pathways.', icon: BookOpen, path: '/scholar', cta: 'Explore Scholar' },
  ];
        ...
        renderItem={(item, index) => (
          ...
          >
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-sm">{item.badge}</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-10 items-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-3">
              {stats.map((stat) => {
                ...
                return (
                  <div key={stat.label} className="flex items-center gap-4 rounded-md border border-slate-200 bg-white px-5 py-4 shadow-sm">
                    <div className={`w-10 h-10 ${stat.bgCls} rounded-sm flex items-center justify-center flex-shrink-0`}>
            ...
            {serviceCategories.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs text-slate-500 group-hover:text-blue-700 transition-colors">{cat.count} services →</span>
                </Link>
              );
            })}
          </div>
            ...
            {events.map((event) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-sm ${eventTypeCls[event.type] ?? 'bg-slate-100 text-slate-600'}`}>
                    {event.type}
                  </span>
                  <span className="text-xs font-semibold text-emerald-700 bg-emerald-50 px-2 py-1 rounded-sm">+50 pts</span>
                </div>
      ...
      <section className="py-14 bg-slate-50 border-y border-slate-200">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {testimonials.map((t) => (
              <div key={t.name} className="bg-white border border-slate-300 rounded-sm p-6 elevate-card flex flex-col gap-4">
