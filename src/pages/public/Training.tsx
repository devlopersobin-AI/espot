...
import { Search, Users, Star, ArrowRight, BookOpen, Calendar, Target } from 'lucide-react';

export default function Training() {
  const subNav = ['Courses', 'Top Trainers', 'Academic Pathways', 'Register Training'];
  ...
  const trainingPrograms = [
    { title: 'Corporate Leadership Training', duration: '2 Days', mode: 'In-person', price: '$499' },
    { title: 'Digital Skills Bootcamp', duration: '4 Weeks', mode: 'Online', price: '$299' },
    { title: 'Executive Coaching Program', duration: '3 Months', mode: 'Hybrid', price: '$1,299' },
    { title: 'Team Performance Workshop', duration: '1 Day', mode: 'In-person', price: '$199' },
  ];
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 w-full">
          ...
          {activeTab === 'Courses' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                <h2 className="text-xl font-bold text-slate-900">Browse Courses</h2>
                <div className="relative w-full sm:w-80">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input value={courseQuery} onChange={(e) => setCourseQuery(e.target.value)} placeholder="Search courses, categories, instructors..." className="w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {filteredCourses.map((course) => (
                  <div key={course.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div className="p-5">
                      <h3 className="font-bold text-base text-slate-900 mb-2 line-clamp-2">{course.title}</h3>
                      <p className="text-sm text-slate-500 mb-4">By {course.instructor}</p>
                      <div className="flex items-center justify-between mb-4">
                        <div className="font-bold text-base text-slate-900">{course.price}</div>
                      ...
                      <button
                        type="button"
                        onClick={() => setApplicationTarget(`Enroll: ${course.title}`)}
          ...
          {activeTab === 'Top Trainers' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                <h2 className="text-xl font-bold text-slate-900">Featured Instructors</h2>
                <div className="relative w-full sm:w-72">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input value={trainerQuery} onChange={(e) => setTrainerQuery(e.target.value)} placeholder="Search trainers or specialties..." className="w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {filteredTrainers.map((trainer) => (
                  <div key={trainer.id} className="bg-white border border-slate-200 rounded-2xl p-6 text-center hover:shadow-lg hover:border-blue-300 transition-all group">
                    <Link to={`/profile/trainer/${trainer.id}`} className="block">
                      ...
                      <h3 className="font-bold text-base text-slate-900 group-hover:text-blue-600 transition-colors">{trainer.name}</h3>
                    ...
                    <button
                      type="button"
                      onClick={() => setApplicationTarget(`Train with ${trainer.name}`)}
              ...
              {filteredTrainers.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">No trainers match your search.</div>}
              {applicationTarget && (
                <div ref={applicationFormRef} tabIndex={-1} className="max-w-3xl focus:outline-none">
                  <QuickRegisterForm
                    heading="Training Registration"
                    targetLabel={applicationTarget}
          ...
          {activeTab === 'Academic Pathways' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto">
              {applicationTarget && (
                <div ref={applicationFormRef} tabIndex={-1} className="mb-6 focus:outline-none">
                  <QuickRegisterForm
                    heading="Pathway Enrollment"
                    targetLabel={applicationTarget}
                    submitLabel="Enroll Now"
                    onClose={() => setApplicationTarget(null)}
                  />
              ...
              <div className="text-center mb-8">
                <h2 className="text-xl font-bold text-slate-900 mb-2">Map Your Educational Journey</h2>
                  ...
                  <div key={i} className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-slate-600 mb-4 text-sm">{pathway.desc}</p>
                      ...
                      <button type="button" onClick={() => setApplicationTarget(`Enroll: ${pathway.title}`)} className="shrink-0 bg-slate-900 text-white font-bold py-2.5 px-5 rounded-xl hover:bg-slate-800 transition-colors flex items-center gap-2 text-sm">
          ...
          {activeTab === 'Register Training' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto space-y-8">
              <div className="text-center">
                <h2 className="text-xl font-bold text-slate-900 mb-2">Register for Training</h2>
                <p className="text-slate-600">Choose a training program and submit your registration to get started.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {trainingPrograms.map((program, i) => (
                  <div key={i} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
                    <h3 className="font-bold text-base text-slate-900 mb-3">{program.title}</h3>
                    <div className="flex flex-wrap gap-2 mb-5 text-xs font-medium">
                      <span className="bg-slate-100 text-slate-600 px-2.5 py-1 rounded-full">{program.duration}</span>
                      <span className="bg-blue-50 text-blue-700 px-2.5 py-1 rounded-full">{program.mode}</span>
                      <span className="bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-full font-bold">{program.price}</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => setApplicationTarget(program.title)}
