...
import { ArrowRight, Banknote, MapPin, Sparkles, TrendingUp } from 'lucide-react';
...
export default function InvestmentDetail() {
  ...
  const { summary } = item;
  const progress = Math.max(8, Math.min(100, Math.round((summary.raised / summary.target) * 100)));
      ...
      <div className="bg-gradient-to-br from-emerald-950 via-teal-900 to-cyan-900 text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-3">
            <Metric label="Stage" value={summary.stage} icon={<TrendingUp className="w-4 h-4" />} />
            <Metric label="Location" value={summary.location} icon={<MapPin className="w-4 h-4" />} />
            <Metric label="Ticket" value={summary.ticket} icon={<Banknote className="w-4 h-4" />} />
            <Metric label="Investor Fit" value={summary.fit} icon={<Sparkles className="w-4 h-4" />} />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] gap-6">
          <div className="space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
              ...
              <p className="mt-3 text-sm text-slate-700 leading-relaxed">{item.thesis}</p>
            </section>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
              ...
              <ul className="mt-3 space-y-2">
                {item.useOfFunds.map((point) => (
                  <li key={point} className="text-sm text-slate-700 flex items-start gap-2">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
              <h2 className="text-xl font-semibold text-slate-900">Risks and Mitigation</h2>
              <ul className="mt-3 space-y-2">
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 h-fit lg:sticky lg:top-[calc(var(--site-header-height)+16px)]">
            <h3 className="text-lg font-semibold text-slate-900">Round Progress</h3>
            ...
            <div className="mt-5">
              <h4 className="text-sm font-semibold text-slate-900">Support Model</h4>
            ...
            <div className="mt-6 space-y-2.5">
              <Link to="/contact" className="w-full inline-flex justify-center items-center gap-1.5 rounded-xl bg-emerald-600 text-white px-4 py-3 text-sm font-semibold hover:bg-emerald-700 transition-colors">
                Request Intro <ArrowRight className="w-4 h-4" />
              </Link>
              <Link to="/investment" className="w-full inline-flex justify-center items-center rounded-xl border border-slate-300 text-slate-700 px-4 py-3 text-sm font-semibold hover:bg-slate-50 transition-colors">
                Back to Investment
              </Link>
            </div>
          </aside>
